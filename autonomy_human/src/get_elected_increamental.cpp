...
//        if(msg.is_valid) check_election = true;
//        else if(!msg.is_valid) check_election = false;
}
                ...
                if((last_face_info.toSec() > FACESCORE_TIMEOUT) || (face_info.faceScore == 0)) // No - It can not see any face
               {
                ...
                else // Yes - It can see a face!
                {
                    ...
                    else // Yes - It can get proper election result --> Everything is ready for decision on election.
                    {
                        ...
                        if ((talker_last_ts.toSec() < SPEECH_TIMEOUT) && (strcmp(talker.speech.data.c_str(),"start election") == 0))// if yes --> reset the election
                        {
                            is_elected = false;
                        }
